...
# read performer_id list
# return performer_id
# return performer_name
# return performer_img_url
...
if __name__ == '__main__':
    ...
    producer_list = open("producer_list_pt_2.txt")

    # create and write to csvs & text files; csvs for better view of txt python will use
    # producer data
    with open('producer_data_scrape_txt_hb.txt', 'w') as f:
        f.write("producer_id, producer_name, producer_img_url\n")

    with open('producer_data_scrape_csv_hb.csv', 'w') as f:
        f.write("producer_id, producer_name, producer_img_url\n")

    #song data
    with open('song_data_scrape_txt_hb.txt', 'w') as f:
    ...
    with open('song_data_scrape_csv_hb.csv', 'w') as f:
        f.write("song_id, song_title, album_id, performer_id, release_date, release_year, release_month, release_day, apple_music_player_url\n")
    ...
    for producer in producer_list:
        ...
        if get_producer_id(producer_name) != "id not found":
            ...
            with open('producer_data_scrape_txt_hb.txt', 'a') as f:
            ...
            with open('producer_data_scrape_csv_hb.csv', 'a') as f:
                for data in producer_data:
                    f.write(
                        str(producer_id) + "," + str(producer_data_components[0]) + 
                        "," + str(producer_data_components[1]) + "," + 
            ...
            with open('song_data_scrape_txt_hb.txt', 'a') as f:
                for song in songs:
            ...
            with open('song_data_scrape_csv_hb.csv', 'a') as f:
                for song in songs:
                    split_songs = song.split("^")
                    f.write(
                        str(split_songs[0]) + "," + str(split_songs[1]) + "," + 
